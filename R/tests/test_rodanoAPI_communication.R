...
test_that("add_scope", {
    gen_scopecode <- function() paste(sample(0:9, 10, T), collapse = "")
...
test_that("add_user", {
    gen_email <- function() paste0(c(sample(0:9, 10, T), "@rodano.ch"), collapse = "")

    # Just create and invite the user

    output_1 <- add_user("Test User", gen_email(), DATAENTRY_ID, SCOPE_PK_LV2, URLBASE, AUTH, activate = F, pwd = NULL)

    # Also activate the user

    output_2 <- add_user("Test User", gen_email(), DATAENTRY_ID, SCOPE_PK_LV2, URLBASE, AUTH, activate = T, pwd = "TestPwd01!")
})
...
test_that("get_extract", {
    ...
    output_2 <- get_extract(URLBASE, AUTH, DATASET_ID, includeModifDate = T)
